...
function OrderRow({ order, onOpenTicket }: { order: CustomerOrder; onOpenTicket: (orderNo?: string) => void }) {
  ...
  return <article className="account-order-row">
    ...
    <details className="account-order-details">
      ...
      <div className="account-order-detail-grid"><div><span>支付方式</span><strong>{order.paymentMethod === "wechat" ? "微信支付" : order.paymentMethod === "bepusdt" ? "USDT / 加密货币" : "支付宝"}</strong></div><div><span>创建时间</span><strong>{dateLabel(order.createdAt)}</strong></div><div><span>支付时间</span><strong>{order.paidAt ? dateLabel(order.paidAt) : "尚未支付"}</strong></div><div><span>订单状态</span><strong>{statusLabels[order.status]}</strong></div></div>
