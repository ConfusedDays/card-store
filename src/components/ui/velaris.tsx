...
void main() {
  ...
  col += u_colors[1] * smoothstep(0.8, 0.0, dist) * 0.18;
...
const fallbackColors = ["#d9f3ed", "#9ed8cc", "#5fb6a5", "#f4f6f3"];
...
export default function Velaris({
  ...
  useLayoutEffect(() => {
    ...
    const colorsUniform = gl.getUniformLocation(program, "u_colors");
    const bgUniform = gl.getUniformLocation(program, "u_bg");
    const palette = [...colors, ...fallbackColors].slice(0, 4);
    ...
    const render = (now: number) => {
      ...
      gl.uniform1f(time, now * 0.001 * speed);
      gl.uniform1f(grainUniform, grain);
      gl.uniform3f(bgUniform, ...hexToRgb(bg));
      gl.uniform3fv(colorsUniform, flatColors);
  ...
  return (
    <div ref={containerRef} style={{ height }} className={cn("relative w-full overflow-hidden", className)} aria-hidden="true">
