...
export function AdminDashboard() {
  ...
  async function mutateInventory(action: "available" | "disabled" | "delete") {
    const ids = selectedKeyIds.filter((id) => inventoryKeys.some((key) => key.id === id && (key.status === "available" || key.status === "disabled")));
    if (!ids.length) {
      setInventoryMessage("已售和预留卡密可以选中查看，但不能修改或删除。");
      return;
    }
  ...
  const selectedReadonlyKeyCount = selectedKeyIds.length - selectedEditableKeyIds.length;
        ...
        <section className="admin-section" id="inventory">
          ...
          <div className="inventory-manager table-shell">
            <div className="inventory-manager-toolbar">
              <div><h3><KeyRound size={18} />卡密管理</h3><p>全部卡密均可选中查看；已售与预留卡密保持只读，避免影响已交付订单。</p></div>
            ...
            <div className="inventory-bulk-actions" aria-live="polite"><span>已选 <strong>{selectedKeyIds.length}</strong> 条</span>{selectedKeyIds.length > 0 && <small>{selectedEditableKeyIds.length ? `其中 ${selectedEditableKeyIds.length} 条可操作` : "所选卡密均为只读"}{selectedReadonlyKeyCount ? `，${selectedReadonlyKeyCount} 条已售/预留仅供查看` : ""}</small>}<div className="inventory-bulk-buttons"><button type="button" className="inventory-bulk-button inventory-enable-button" disabled={!selectedEditableKeyIds.length || inventoryPending} onClick={() => void mutateInventory("available")}><CheckCircle2 size={16} />启用</button><button type="button" className="inventory-bulk-button inventory-disable-button" disabled={!selectedEditableKeyIds.length || inventoryPending} onClick={() => void mutateInventory("disabled")}><CircleOff size={16} />停用</button><button type="button" className="inventory-bulk-button inventory-delete-button" disabled={!selectedEditableKeyIds.length || inventoryPending} onClick={() => void mutateInventory("delete")}><Trash2 size={16} />删除</button></div></div>
