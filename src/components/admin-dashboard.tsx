...
export function AdminDashboard() {
  ...
  async function mutateInventory(action: "available" | "disabled" | "sold" | "delete") {
    ...
    if (action === "sold" && !window.confirm(`确认将选中的 ${ids.length} 条卡密标记为已用？\n\n标记后不会再被自动发卡。`)) return;
    ...
    try {
      ...
      const resultMessage = action === "delete" ? `已删除 ${data.deleted} 条卡密${data.detachedDeliveries ? `，移除 ${data.detachedDeliveries} 条订单发卡关联` : ""}` : `${action === "available" ? "已启用" : action === "sold" ? "已标记为已用" : "已停用"} ${data.updated} 条卡密`;
    ...
    return (
      <div className="admin-shell">
        ...
        <form onSubmit={(event) => { event.preventDefault(); void loadOverview(); }}>
          ...
          <button className="primary-button" disabled={loading}><AnimatedButtonIcon loading={loading} idle={<LogIn size={18} />} />进入后台</button>
  ...
  return (
    <div className="admin-shell">
      ...
      <div className="admin-content">
        <header><div><span className="section-index">OPERATIONS</span><h1>销售与库存</h1></div><div className="admin-header-actions"><button className="icon-action admin-refresh-button" onClick={() => loadOverview()} title="刷新数据"><AnimatedButtonIcon loading={loading} idle={<RefreshCw size={18} />} /></button></div></header>
        {error && <p className="form-error">{error}</p>}
        ...
        <section className="admin-section" id="backups">
          ...
          <div className="backup-layout">
            ...
            <form className="backup-panel restore-panel" onSubmit={restoreBackup}>
              ...
              <button className="danger-button" disabled={restoreBusy || restoreConfirmation !== "恢复数据"}><AnimatedButtonIcon loading={restoreBusy} idle={<ArchiveRestore size={18} />} />{restoreBusy ? "正在恢复..." : "恢复此备份"}</button>
            </form>
          </div>
        </section>
        ...
        <section className="admin-section" id="orders">
          ...
          <div className="order-recycle-bin table-shell" id="order-recycle-bin">
            ...
            <div className="order-recycle-table"><table><thead><tr><th>订单号</th><th>客户</th><th>金额</th><th>订单状态</th><th>移入时间</th><th>操作</th></tr></thead><tbody>
              {recycledOrders.length ? recycledOrders.map((order) => <tr key={order.orderNo}><td><code>{order.orderNo}</code></td><td>{order.email}</td><td>{money(order.amountCents)}</td><td><span className={`status-badge status-${order.status}`}>{order.status}</span></td><td>{new Date(order.deletedAt.replace(" ", "T") + "Z").toLocaleString("zh-CN")}</td><td className="order-action-cell"><div className="order-action-buttons"><button className="resend-email-button" type="button" onClick={() => void restoreOrder(order.orderNo)} title="恢复订单"><AnimatedButtonIcon idle={<Undo2 size={15} />} /><span>恢复</span></button><button className="order-purge-button" type="button" onClick={() => void permanentlyDeleteOrder(order.orderNo)} title="永久删除"><AnimatedButtonIcon idle={<Trash2 size={15} />} /><span>永久删除</span></button></div></td></tr>) : <tr><td colSpan={6} className="empty-cell">回收站为空</td></tr>}
            </tbody></table></div>
          </div>
        </section>
