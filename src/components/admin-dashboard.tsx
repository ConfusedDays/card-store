...
import { ArchiveRestore, ArrowLeft, Boxes, CheckCheck, CheckCircle2, CircleDollarSign, CircleOff, Copy, Download, KeyRound, LoaderCircle, LogIn, MailCheck, Menu as MenuIcon, PackagePlus, ReceiptText, RefreshCw, Search, Send, ShieldCheck, Tags, Trash2, TriangleAlert, Undo2, Upload } from "lucide-react";
      ...
      <div className="admin-content">
        ...
        <section className="admin-section" id="inventory">
          ...
          <div className="inventory-manager table-shell">
            ...
            <div className="inventory-bulk-actions" aria-live="polite"><span>已选 <strong>{selectedKeyIds.length}</strong> 条</span>{selectedKeyIds.length > 0 && <small>所有状态均可批量操作</small>}<div className="inventory-bulk-buttons"><button type="button" className="inventory-bulk-button inventory-enable-button" disabled={!selectedKeyIds.length || inventoryPending} onClick={() => void mutateInventory("available")}><CheckCircle2 size={16} />启用</button><button type="button" className="inventory-bulk-button inventory-disable-button" disabled={!selectedKeyIds.length || inventoryPending} onClick={() => void mutateInventory("disabled")}><CircleOff size={16} />停用</button><button type="button" className="inventory-bulk-button inventory-used-button" disabled={!selectedKeyIds.length || inventoryPending} onClick={() => void mutateInventory("sold")}><CheckCheck size={16} />已用</button><button type="button" className="inventory-bulk-button inventory-delete-button" disabled={!selectedKeyIds.length || inventoryPending} onClick={() => void mutateInventory("delete")}><Trash2 size={16} />删除</button></div></div>
