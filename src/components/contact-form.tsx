...
export function ContactForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/contact", {
        ...
        body: JSON.stringify({ name: values.get("name"), email: values.get("email"), orderNo: values.get("orderNo"), message: values.get("message") }),
      });
  ...
  return <form className="contact-form" onSubmit={submit}>
    ...
    <label><span>订单号</span><input name="orderNo" required maxLength={80} placeholder="例如 KXXXXXX" /></label>
